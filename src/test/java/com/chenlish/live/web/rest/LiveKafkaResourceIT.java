...
import com.chenlish.live.config.KafkaProperties;
...
class LiveKafkaResourceIT {
    ...
    void setup() {
        ...
        LiveKafkaResource kafkaResource = new LiveKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/live-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/live-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
